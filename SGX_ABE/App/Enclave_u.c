...
sgx_status_t ecall_init_system(sgx_enclave_id_t eid, sgx_status_t* retval, PK* pk)
{
	sgx_status_t status;
	ms_ecall_init_system_t ms;
	ms.ms_pk = pk;
...
sgx_status_t ecall_generate_user_key(sgx_enclave_id_t eid, sgx_status_t* retval, const char** attrs, size_t attr_count, uint64_t expire_time, SK* sk)
{
	sgx_status_t status;
	ms_ecall_generate_user_key_t ms;
	ms.ms_attrs = attrs;
	ms.ms_attr_count = attr_count;
	ms.ms_expire_time = expire_time;
	ms.ms_sk = sk;
...
sgx_status_t ecall_verify_and_transform(sgx_enclave_id_t eid, sgx_status_t* retval, ciphertext_t* ct, SK* sk, merkle_proof_t* proof, ciphertext_t* ct_inter)
{
	sgx_status_t status;
	ms_ecall_verify_and_transform_t ms;
	ms.ms_ct = ct;
	ms.ms_sk = sk;
	ms.ms_proof = proof;
	ms.ms_ct_inter = ct_inter;
